...
type Cell = { x: number; docY: number; collected: boolean };
...
function drawBlock(
  ...
  for (let px = bx + 6; px < bx + bw - 3; px += 10) ctx.fillRect(px, by + 3, 2, 2);
  ctx.fillStyle = "rgba(0,0,0,0.22)";
  ctx.fillRect(bx, by + BLOCK_H - 1, bw, 1);
  ctx.restore();
}

function drawBrainCell(
  ctx: CanvasRenderingContext2D,
  ex: number,
  ey: number,
  scrollY: number,
...
function generateCellLayout(
  nb: number,
  vw: number,
  avoidZones: { top: number; bot: number }[] = [],
) {
  const clearZone = (docY: number) => {
    let y = docY;
  ...
  const cells: Cell[] = [];
  const extraLedges: Block[] = [];
  const mk = (x: number, docY: number, w: number): Block => ({
    x,
    docY,
    w,
    isSpawn: false,
...
export default function RobotGame({ active }: { active: boolean }) {
  ...
  const getPlatforms = useCallback(() => {
    ...
    const platforms: { x: number; y: number; w: number }[] = blocksRef.current
    ...
    PLATFORM_SELECTORS.forEach((sel) => {
      document.querySelectorAll(sel).forEach((el) => {
        ...
        if (docTop > scrollY + window.innerHeight + 200 || docTop + r.height < scrollY - 200)
          return;
  ...
  useEffect(() => {
    ...
    const allDomZones: { top: number; bot: number }[] = [];
    const textZones: { top: number; bot: number }[] = [];
    PLATFORM_SELECTORS.forEach((sel) => {
      document.querySelectorAll(sel).forEach((el) => {
        const r = (el as HTMLElement).getBoundingClientRect();
    ...
    const adjustForText = (docY: number) => {
      let y = docY;
      for (let i = 0; i < 4; i++) {
    ...
    const mkS = (b: { x: number; docY: number; w: number }): Block => ({
      ...b,
      isSpawn: false,
      isGoal: false,
      alpha: 0,
    ...
    blocksRef.current = [
      ...
      ...rawScatter.map((b) => ({ ...b, docY: adjustForText(b.docY) })).map(mkS),
      ...extraLedges,
    ];
      ...
      {gameStatus === "won" && (
        <div className="robot-game-status robot-game-status--won">
          ...
          <div className="robot-game-status-sub">
            all {CELL_COUNT} brain cells recovered
          </div>
          <button className="robot-game-status-btn" onClick={restart}>
            play again
          </button>
